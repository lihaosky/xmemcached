...
import net.rubyeye.xmemcached.impl.ThreeRandomMemcachedSessionLocator;
...
public class XMemcachedClient implements XMemcachedClientMBean, MemcachedClient {
    ...
    @SuppressWarnings("unchecked")
    private final <T> GetsResponse<T> gets0(final String key, final byte[] keyBytes, final Transcoder<T> transcoder)
            throws MemcachedException, TimeoutException, InterruptedException {
        GetsResponse<T> result =
                (GetsResponse<T>) this.fetch0(key, keyBytes, CommandType.GETS_ONE, this.opTimeout, transcoder);
        return result;
    }


    private final void sendCommand(final Command cmd, boolean isSet) throws MemcachedException {
        if (this.shutdown) {
            throw new MemcachedException("Xmemcached is stopped");
        }
        this.connector.send(cmd, isSet);
    }
    ...
    public XMemcachedClient(final String host, final int port, int weight) throws IOException {
        ...
        this.buildConnector(new ArrayMemcachedSessionLocator(this.controllerHostname, this.controllerPort, true, "random", 1), new SimpleBufferAllocator(),
        ...
        this.start0();
        this.connect(new InetSocketAddressWrapper(this.newSocketAddress(host, port), this.serverOrderCount
            .incrementAndGet(), weight, null));
    }
    ...
    public final void addServer(final String server, final int port, int weight) throws IOException {
        ...
        this.connect(new InetSocketAddressWrapper(this.newSocketAddress(server, port), this.serverOrderCount
            .incrementAndGet(), weight, null));
    }
    ...
    public final void addServer(final InetSocketAddress inetSocketAddress, int weight) throws IOException {
        ...
        this.connect(new InetSocketAddressWrapper(inetSocketAddress, this.serverOrderCount.incrementAndGet(), weight,
            null));
    }
    ...
    public final void addServer(String hostList) throws IOException {
        ...
        if (addresses != null && addresses.size() > 0) {
            for (Map.Entry<InetSocketAddress, InetSocketAddress> entry : addresses.entrySet()) {
                ...
                this.connect(new InetSocketAddressWrapper(mainNodeAddr, this.serverOrderCount.incrementAndGet(), 1,
                    null));
                if (standbyNodeAddr != null) {
                    this.connect(new InetSocketAddressWrapper(standbyNodeAddr, this.serverOrderCount.incrementAndGet(),
                        1, mainNodeAddr));
                }
            }
        }
    ...
    private void connect(final InetSocketAddressWrapper inetSocketAddressWrapper) throws IOException {
        ...
        for (int i = 0; i < this.connectionPoolSize; i++) {
            ...
            if (!connected) {
                ...
                this.connector.addToWatingQueue(new ReconnectRequest(inetSocketAddressWrapper, 0, this
    ...
    @SuppressWarnings("unchecked")
    private final <T> Object fetch0(final String key, final byte[] keyBytes, final CommandType cmdType,
            final long timeout, Transcoder<T> transcoder) throws InterruptedException, TimeoutException,
    ...
    private void buildConnector(MemcachedSessionLocator locator, BufferAllocator bufferAllocator,
        ...
        if (locator == null) {
            locator = new ArrayMemcachedSessionLocator(this.controllerHostname, this.controllerPort, false, "random", 1);
        ...
        log.warn("XMemcachedClient use " + this.commandFactory.getProtocol().name() + " protocol");
        this.commandFactory.setBufferAllocator(bufferAllocator);
        this.shutdown = true;
        this.transcoder = transcoder;
        this.sessionLocator = locator;
        this.connector =
                this.newConnector(bufferAllocator, configuration, this.sessionLocator, this.commandFactory,
    ...
    protected MemcachedConnector newConnector(BufferAllocator bufferAllocator, Configuration configuration,
            MemcachedSessionLocator memcachedSessionLocator, CommandFactory commandFactory, int i) {
        // make sure dispatch message thread count is zero
        configuration.setDispatchMessageThreadCount(0);
        return new MemcachedConnector(configuration, memcachedSessionLocator, bufferAllocator, commandFactory, i);
    }
    ...
    private final void registerMBean() {
        if (this.shutdown) {
            XMemcachedMbeanServer.getInstance().registMBean(
                this,
                this.getClass().getPackage().getName() + ":type=" + this.getClass().getSimpleName() + "-"
    ...
    public XMemcachedClient(final InetSocketAddress inetSocketAddress, int weight) throws IOException {
        ...
        this.buildConnector(new ArrayMemcachedSessionLocator(this.controllerHostname, this.controllerPort, false, "random", 1), new SimpleBufferAllocator(),
            XMemcachedClientBuilder.getDefaultConfiguration(), XMemcachedClientBuilder.getDefaultSocketOptions(),
            new TextCommandFactory(), new SerializingTranscoder());
        this.start0();
        this.connect(new InetSocketAddressWrapper(inetSocketAddress, this.serverOrderCount.incrementAndGet(), weight,
    ...
    public XMemcachedClient(String controllerHostname, int controllerPort, boolean connectController, String policy, int interval) throws IOException {
    	
        super();
        this.controllerHostname = controllerHostname;
        this.controllerPort = controllerPort;
        this.buildConnector(new ArrayMemcachedSessionLocator(controllerHostname, controllerPort, connectController, policy, interval), new SimpleBufferAllocator(),
            XMemcachedClientBuilder.getDefaultConfiguration(), XMemcachedClientBuilder.getDefaultSocketOptions(),
            new TextCommandFactory(), new SerializingTranscoder());
        this.start0();
    }
    
    public XMemcachedClient(int copyNum) throws IOException {
    	super();
    	this.buildConnector(new ThreeRandomMemcachedSessionLocator(copyNum), new SimpleBufferAllocator(),
                XMemcachedClientBuilder.getDefaultConfiguration(), XMemcachedClientBuilder.getDefaultSocketOptions(),
                new TextCommandFactory(), new SerializingTranscoder());
         this.start0();
    }
    ...
    XMemcachedClient(MemcachedSessionLocator locator, BufferAllocator allocator, Configuration conf,
        ...
        if (addressMap != null && weights != null) {
            ...
            for (Map.Entry<InetSocketAddress, InetSocketAddress> entry : addressMap.entrySet()) {
                final InetSocketAddress mainNodeAddr = entry.getKey();
                final InetSocketAddress standbyNodeAddr = entry.getValue();
                this.connect(new InetSocketAddressWrapper(mainNodeAddr, this.serverOrderCount.incrementAndGet(),
                    weights[i], null));
                if (standbyNodeAddr != null) {
                    this.connect(new InetSocketAddressWrapper(standbyNodeAddr, this.serverOrderCount.incrementAndGet(),
    ...
    private final void optimiezeSetReadThreadCount(Configuration conf, int addressCount) {
        if (conf != null && addressCount > 1) {
            if (this.isLinuxPlatform() && conf.getReadThreadCount() == DEFAULT_READ_THREAD_COUNT) {
    ...
    public XMemcachedClient(List<InetSocketAddress> addressList) throws IOException {
        ...
        this.buildConnector(new ArrayMemcachedSessionLocator(this.controllerHostname, this.controllerPort, false, "random", 1), simpleBufferAllocator,
            XMemcachedClientBuilder.getDefaultConfiguration(), XMemcachedClientBuilder.getDefaultSocketOptions(),
            new TextCommandFactory(), new SerializingTranscoder());
        this.start0();
        for (InetSocketAddress inetSocketAddress : addressList) {
            this.connect(new InetSocketAddressWrapper(inetSocketAddress, this.serverOrderCount.incrementAndGet(), 1,
                null));
     ...
     */
    @SuppressWarnings("unchecked")
    public final <T> T get(final String key, final long timeout) throws TimeoutException, InterruptedException,
            MemcachedException {
     ...
     * net.rubyeye.xmemcached.transcoders.Transcoder)
     */
    public final <T> T get(final String key, final Transcoder<T> transcoder) throws TimeoutException,
    ...
    private <T> Object get0(String key, final long timeout, final CommandType cmdType, final Transcoder<T> transcoder)
            throws TimeoutException, InterruptedException, MemcachedException {
     ...
     */
    public final <T> GetsResponse<T> gets(final String key) throws TimeoutException, InterruptedException,
            MemcachedException {
     ...
     */
    @SuppressWarnings("unchecked")
    public final <T> GetsResponse<T> gets(final String key, final long timeout) throws TimeoutException,
            InterruptedException, MemcachedException {
     ...
     */
    @SuppressWarnings("unchecked")
    public final <T> GetsResponse<T> gets(final String key, final Transcoder transcoder) throws TimeoutException,
            InterruptedException, MemcachedException {
    ...
    public final <T> Map<String, T> get(final Collection<String> keyCollections, final long timeout,
            final Transcoder<T> transcoder) throws TimeoutException, InterruptedException, MemcachedException {
     ...
     * @see net.rubyeye.xmemcached.MemcachedClient#get(java.util.Collection)
     */
    public final <T> Map<String, T> get(final Collection<String> keyCollections) throws TimeoutException,
            InterruptedException, MemcachedException {
    ...
    public final <T> Map<String, GetsResponse<T>> gets(final Collection<String> keyCollections, final long timeout,
            final Transcoder<T> transcoder) throws TimeoutException, InterruptedException, MemcachedException {
        return (Map<String, GetsResponse<T>>) this
            .getMulti0(keyCollections, timeout, CommandType.GETS_MANY, transcoder);
    }
    ...
    private final <T> Map<String, T> getMulti0(final Collection<String> keys, final long timeout,
            final CommandType cmdType, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException,
            MemcachedException {
        ...
        for (Command getCmd : commands) {
            ...
            if (cmdType == CommandType.GET_MANY) {
                ...
                while (it.hasNext()) {
                    ...
                    if (this.sanitizeKeys) {
                        result.put(this.decodeKey(entry.getKey()), transcoder.decode(entry.getValue()));
                    }
                    else {
                        result.put(entry.getKey(), transcoder.decode(entry.getValue()));
                    }
                }
    ...
    private final Collection<List<String>> catalogKeys(final Collection<String> keyCollections) {
        ...
        for (String key : keyCollections) {
            Session index = this.sessionLocator.getSessionByKey(key, false);
     ...
     * @return
     */
    private final Collection<Map<String, Integer>> catalogKeys(final Map<String, Integer> keyExpMap) {
        ...
        for (Map.Entry<String, Integer> entry : keyExpMap.entrySet()) {
            ...
            Session index = this.sessionLocator.getSessionByKey(key, false);
            if (!catalogMap.containsKey(index)) {
                Map<String, Integer> tmpKeyExpMap = new HashMap<String, Integer>(100);
    ...
    private final <T> Command sendGetMultiCommand(final Collection<String> keys, final CountDownLatch latch,
            final CommandType cmdType, final Transcoder<T> transcoder) throws InterruptedException, TimeoutException,
            MemcachedException {
     ...
     */
    public final <T> boolean set(String key, final int exp, final T value, final Transcoder<T> transcoder,
        ...
        return this.sendStoreCommand(
            this.commandFactory.createSetCommand(key, keyBytes, exp, value, false, transcoder), timeout);
    }
    ...
    public <T> void setWithNoReply(String key, int exp, T value, Transcoder<T> transcoder) throws InterruptedException,
        ...
        byte[] keyBytes = this.checkStoreArguments(key, exp, value);
        try {
            this.sendStoreCommand(this.commandFactory.createSetCommand(key, keyBytes, exp, value, true, transcoder),
                this.opTimeout);
        }
        catch (TimeoutException e) {
     ...
     * java.lang.Object)
     */
    public final boolean set(final String key, final int exp, final Object value) throws TimeoutException,
            InterruptedException, MemcachedException {
     ...
     * net.rubyeye.xmemcached.transcoders.Transcoder, long)
     */
    public final <T> boolean add(String key, final int exp, final T value, final Transcoder<T> transcoder,
            final long timeout) throws TimeoutException, InterruptedException, MemcachedException {
        key = this.sanitizeKey(key);
        byte[] keyBytes = this.checkStoreArguments(key, exp, value);
        return this.sendStoreCommand(
            this.commandFactory.createAddCommand(key, keyBytes, exp, value, false, transcoder), timeout);
    }
     ...
     * java.lang.Object)
     */
    public final boolean add(final String key, final int exp, final Object value) throws TimeoutException,
            InterruptedException, MemcachedException {
    ...
    public <T> void addWithNoReply(String key, int exp, T value, Transcoder<T> transcoder) throws InterruptedException,
        ...
        byte[] keyBytes = this.checkStoreArguments(key, exp, value);
        try {
            this.sendStoreCommand(this.commandFactory.createAddCommand(key, keyBytes, exp, value, true, transcoder),
                this.opTimeout);
        }
        catch (TimeoutException e) {
            throw new MemcachedException(e);
    ...
    public <T> void replaceWithNoReply(String key, int exp, T value, Transcoder<T> transcoder)
        ...
        try {
            this.sendStoreCommand(
                this.commandFactory.createReplaceCommand(key, keyBytes, exp, value, true, transcoder), this.opTimeout);
        }
        catch (TimeoutException e) {
            throw new MemcachedException(e);
        }
     ...
     */
    public final <T> boolean replace(String key, final int exp, final T value, final Transcoder<T> transcoder,
            final long timeout) throws TimeoutException, InterruptedException, MemcachedException {
        key = this.sanitizeKey(key);
        byte[] keyBytes = this.checkStoreArguments(key, exp, value);
        return this.sendStoreCommand(
            this.commandFactory.createReplaceCommand(key, keyBytes, exp, value, false, transcoder), timeout);
    }
     ...
     */
    public final boolean replace(final String key, final int exp, final Object value) throws TimeoutException,
            InterruptedException, MemcachedException {
     ...
     * java.lang.Object)
     */
    public final boolean append(final String key, final Object value) throws TimeoutException, InterruptedException,
            MemcachedException {
     ...
     * java.lang.Object, long)
     */
    public final boolean append(String key, final Object value, final long timeout) throws TimeoutException,
            InterruptedException, MemcachedException {
        key = this.sanitizeKey(key);
        byte[] keyBytes = this.checkStoreArguments(key, 0, value);
        return this.sendStoreCommand(
            this.commandFactory.createAppendCommand(key, keyBytes, value, false, this.transcoder), timeout);
    }
    ...
    public void appendWithNoReply(String key, Object value) throws InterruptedException, MemcachedException {
        ...
        byte[] keyBytes = this.checkStoreArguments(key, 0, value);
        try {
            this.sendStoreCommand(this.commandFactory.createAppendCommand(key, keyBytes, value, true, this.transcoder),
                this.opTimeout);
        }
        catch (TimeoutException e) {
            throw new MemcachedException(e);
     ...
     * java.lang.Object)
     */
    public final boolean prepend(final String key, final Object value) throws TimeoutException, InterruptedException,
            MemcachedException {
     ...
     * java.lang.Object, long)
     */
    public final boolean prepend(String key, final Object value, final long timeout) throws TimeoutException,
            InterruptedException, MemcachedException {
        key = this.sanitizeKey(key);
        byte[] keyBytes = this.checkStoreArguments(key, 0, value);
        return this.sendStoreCommand(
            this.commandFactory.createPrependCommand(key, keyBytes, value, false, this.transcoder), timeout);
    }
    ...
    public void prependWithNoReply(String key, Object value) throws InterruptedException, MemcachedException {
        ...
        try {
            this.sendStoreCommand(
                this.commandFactory.createPrependCommand(key, keyBytes, value, true, this.transcoder), this.opTimeout);
        }
        catch (TimeoutException e) {
            throw new MemcachedException(e);
        }
     ...
     */
    public final <T> boolean cas(String key, final int exp, final T value, final Transcoder<T> transcoder,
            final long timeout, final long cas) throws TimeoutException, InterruptedException, MemcachedException {
        key = this.sanitizeKey(key);
        byte[] keyBytes = this.checkStoreArguments(key, 0, value);
        return this.sendStoreCommand(
            this.commandFactory.createCASCommand(key, keyBytes, exp, value, cas, false, transcoder), timeout);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public final boolean cas(final String key, final int exp, final Object value, final long timeout, final long cas)
            throws TimeoutException, InterruptedException, MemcachedException {
     ...
     * net.rubyeye.xmemcached.transcoders.Transcoder, long)
     */
    public final <T> boolean cas(final String key, final int exp, final T value, final Transcoder<T> transcoder,
    ...
    private final <T> boolean cas0(final String key, final int exp, GetsResponse<T> getsResponse,
            final CASOperation<T> operation, final Transcoder<T> transcoder, byte[] keyBytes, boolean noreply)
            throws TimeoutException, InterruptedException, MemcachedException {
        if (operation == null) {
        ...
        while (tryCount <= operation.getMaxTries()
                ...
                && !this.sendStoreCommand(this.commandFactory.createCASCommand(key, keyBytes, exp,
                    operation.getNewValue(result.getCas(), result.getValue()), result.getCas(), noreply, transcoder),
                    this.opTimeout) && !noreply) {
            tryCount++;
     ...
     * net.rubyeye.xmemcached.transcoders.Transcoder)
     */
    public final <T> boolean cas(String key, final int exp, GetsResponse<T> getsReponse,
            final CASOperation<T> operation, final Transcoder<T> transcoder) throws TimeoutException,
            InterruptedException, MemcachedException {
        key = this.sanitizeKey(key);
        byte[] keyBytes = ByteUtils.getBytes(key);
        ByteUtils.checkKey(keyBytes);
     ...
     */
    @SuppressWarnings("unchecked")
    public final <T> boolean cas(final String key, final int exp, GetsResponse<T> getsReponse,
            final CASOperation<T> operation) throws TimeoutException, InterruptedException, MemcachedException {
     ...
     * net.rubyeye.xmemcached.GetsResponse, net.rubyeye.xmemcached.CASOperation)
     */
    public final <T> boolean cas(final String key, GetsResponse<T> getsReponse, final CASOperation<T> operation)
            throws TimeoutException, InterruptedException, MemcachedException {
     ...
     * net.rubyeye.xmemcached.CASOperation)
     */
    public final <T> boolean cas(final String key, final CASOperation<T> operation) throws TimeoutException,
            InterruptedException, MemcachedException {
     ...
     */
    public final boolean delete(final String key, final int time) throws TimeoutException, InterruptedException,
            MemcachedException {
     ...
     */
    public final void deleteWithNoReply(final String key, final int time) throws InterruptedException,
            MemcachedException {
        try {
    ...
    private boolean delete0(String key, final int time, boolean noreply, long opTimeout) throws MemcachedException,
            InterruptedException, TimeoutException {
        key = this.sanitizeKey(key);
        final byte[] keyBytes = ByteUtils.getBytes(key);
        ByteUtils.checkKey(keyBytes);
        final Command command = this.commandFactory.createDeleteCommand(key, keyBytes, time, noreply);
        this.sendCommand(command, false);
     ...
     * @throws MemcachedException
     */
    private <T> Map<String, T> getAndTouch(Map<String, Integer> keyExpMap, long opTimeout) throws TimeoutException,
            InterruptedException, MemcachedException {
        ...
        if (this.sanitizeKeys) {
            ...
            for (Map.Entry<String, Integer> entry : keyExpMap.entrySet()) {
                innerKeyExpMap.put(this.sanitizeKey(entry.getKey()), entry.getValue());
            }
        }
        final CountDownLatch latch;
        final List<Command> commands;
        if (this.connector.getSessionSet().size() <= 1) {
            ...
            Collection<Map<String, Integer>> catalogKeyExps = this.catalogKeys(innerKeyExpMap);
    ...
    private Command sendMultiGATCommand(Map<String, Integer> innerKeyExpMap, CountDownLatch latch)
            throws MemcachedException {
        Iterator<String> it = innerKeyExpMap.keySet().iterator();
        String key = null;
        List<com.google.code.yanf4j.buffer.IoBuffer> bufferList =
                new ArrayList<com.google.code.yanf4j.buffer.IoBuffer>();
        int totalLength = 0;
        while (it.hasNext()) {
            key = it.next();
            if (it.hasNext()) {
                ...
                Command command =
                        this.commandFactory.createGetAndTouchCommand(key, ByteUtils.getBytes(key), null,
        ...
        Command resultCommand = new BinaryGetMultiCommand(key, CommandType.GET_MANY, latch);
        resultCommand.setIoBuffer(mergedBuffer);
        this.sendCommand(resultCommand, false);
        return resultCommand;
    }


    private <T> Map<String, T> getAndTouch(Map<String, Integer> keyExpMap) throws TimeoutException,
            InterruptedException, MemcachedException {
     ...
     */
    public final long incr(String key, final long delta) throws TimeoutException, InterruptedException,
            MemcachedException {
        key = this.sanitizeKey(key);
     ...
     */
    public final long decr(String key, final long delta) throws TimeoutException, InterruptedException,
            MemcachedException {
        key = this.sanitizeKey(key);
    ...
    public final void flushAll(int exptime, long timeout) throws TimeoutException, InterruptedException,
            MemcachedException {
    ...
    private void flushAllMemcachedServers(long timeout, boolean noreply, int exptime) throws MemcachedException,
            InterruptedException, TimeoutException {
    ...
    private void setMemcachedLoggingLevel(InetSocketAddress address, int level, boolean noreply)
        ...
        if (sessionQueue == null || sessionQueue.peek() == null) {
            throw new MemcachedException("could not find session for " + SystemUtils.getRawAddress(address) + ":"
     ...
     * )
     */
    public final void flushAll(InetSocketAddress address) throws MemcachedException, InterruptedException,
            TimeoutException {
     ...
     * , long)
     */
    public final void flushAll(InetSocketAddress address, long timeout) throws MemcachedException,
            InterruptedException, TimeoutException {
        this.flushSpecialMemcachedServer(address, timeout, false, 0);
    }


    public final void flushAll(InetSocketAddress address, long timeout, int exptime) throws MemcachedException,
            InterruptedException, TimeoutException {
        this.flushSpecialMemcachedServer(address, timeout, false, exptime);
    }


    private void flushSpecialMemcachedServer(InetSocketAddress address, long timeout, boolean noreply, int exptime)
            throws MemcachedException, InterruptedException, TimeoutException {
        if (address == null) {
            throw new IllegalArgumentException("Null adderss");
        }
     ...
     */
    public final Map<String, String> stats(InetSocketAddress address) throws MemcachedException, InterruptedException,
            TimeoutException {
     ...
     */
    @SuppressWarnings("unchecked")
    public final Map<String, String> stats(InetSocketAddress address, long timeout) throws MemcachedException,
            InterruptedException, TimeoutException {
        if (address == null) {
            throw new IllegalArgumentException("Null inetSocketAddress");
        }
    ...
    public final Map<InetSocketAddress, Map<String, String>> getStats() throws MemcachedException,
            InterruptedException, TimeoutException {
        return this.getStats(this.opTimeout);
    }


    public final Map<InetSocketAddress, Map<String, String>> getStatsByItem(String itemName) throws MemcachedException,
            InterruptedException, TimeoutException {
    ...
    public final Map<InetSocketAddress, String> getVersions() throws TimeoutException, InterruptedException,
            MemcachedException {
        return this.getVersions(this.opTimeout);
    }


    public final Map<InetSocketAddress, String> getVersions(long timeout) throws TimeoutException,
            InterruptedException, MemcachedException {
        final Set<Session> sessionSet = this.connector.getSessionSet();
    ...
    private long sendIncrOrDecrCommand(final String key, final long delta, long initValue, final CommandType cmdType,
            boolean noreply, long operationTimeout, int exp) throws InterruptedException, TimeoutException,
            MemcachedException {
        final byte[] keyBytes = ByteUtils.getBytes(key);
        ByteUtils.checkKey(keyBytes);
        final Command command =
                this.commandFactory.createIncrDecrCommand(key, keyBytes, delta, initValue, exp, cmdType, noreply);
        this.sendCommand(command, false);
        if (!command.isNoreply()) {
            ...
            if (result instanceof String) {
                if (((String) result).equals("NOT_FOUND")) {
                    if (this.add(key, exp, String.valueOf(initValue), this.opTimeout)) {
                        ...
                        return this.sendIncrOrDecrCommand(key, delta, initValue, cmdType, noreply, operationTimeout,
                            exp);
                    }
                }
                else {
                    throw new MemcachedException("Unknown result type for incr/decr:" + result.getClass() + ",result="
                            + result);
                }
            }
            else {
    ...
    private final <T> boolean sendStoreCommand(Command command, long timeout) throws InterruptedException,
    ...
    private void latchWait(final Command cmd, final long timeout) throws InterruptedException, TimeoutException {
        if (!cmd.getLatch().await(timeout, TimeUnit.MILLISECONDS)) {
            cmd.cancel();
            throw new TimeoutException("Timed out(" + timeout + ") waiting for operation");
        }
    }
    ...
    private String sanitizeKey(String key) throws MemcachedException {
        try {
            ...
            throw new MemcachedException("Unsupport encoding utf-8 when sanitizeKey", e);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public KeyIterator getKeyIterator(InetSocketAddress address) throws MemcachedException, TimeoutException,
            InterruptedException {
        if (address == null) {
